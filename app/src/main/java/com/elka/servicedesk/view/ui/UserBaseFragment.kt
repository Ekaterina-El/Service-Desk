...
import com.elka.servicedesk.view.ui.report.ReportIncidentsWithMissedDeadlinesFragmentDirections
...
abstract class UserBaseFragment : BaseFragment() {
	...
	fun goAccident(accidentId: String, path: Int) {
		val dir = when (path) {
			FROM_ACTIVE_ACCIDENTS_TO_ACCIDENT -> EngineerAccidentsFragmentDirections
        .actionEngineerAccidentsFragmentToAccidentFragment(accidentId)
			FROM_IN_WORK_ACCIDENTS_TO_ACCIDENT -> EngineerAccidentsInWorkFragmentDirections
        .actionEngineerAccidentsInWorkFragmentToAccidentFragment(accidentId)
			FROM_CUSTOMER_ACCIDENTS_TO_ACCIDENT -> CustomerIncidentsRequestsDirections
        .actionCustomerIncidentsRequestsToAccidentFragment2(accidentId)
			FROM_FAIL_ACCIDENTS_TO_ACCIDENT -> AdminFailAccidentsFragmentDirections
        .actionAdminFailAccidentsFragmentToAccidentFragment2(accidentId)
			FROM_REPORT_REQUESTS_BY_DIVISIONS_TO_ACCIDENT -> ReportRequestsByDivisionsDirections
        .actionReportRequestsByDivisionsToAccidentFragment(accidentId)
			FROM_REPORT_INCIDENTS_WITH_MISSED_DEADLINE_TO_ACCIDENT -> ReportIncidentsWithMissedDeadlinesFragmentDirections
        .actionReportIncidentsWithMissedDeadlinesFragmentToAccidentFragment(accidentId)
			else -> return
		}
	...
	companion object {
		...
		const val FROM_REPORT_REQUESTS_BY_DIVISIONS_TO_ACCIDENT = 5
		const val FROM_REPORT_INCIDENTS_WITH_MISSED_DEADLINE_TO_ACCIDENT = 6
	}
